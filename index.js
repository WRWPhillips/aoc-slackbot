...
const {getLeaderBoardFromLocal, getLeaderBoard, writeBoardToLocal} = require("./leaderboardFetch");
...
async function tryCompare() {
	...
	if(diff.length > 0) {
		let prevMembers = Object.keys(prevBoard["members"]);
		let nextMembers = Object.keys(nextBoard["members"]);
		let newMembers = nextMembers.filter( x => !prevMembers.includes(x));
		let newMemberMessages = newMembers.map(member => {return `${nextBoard["members"][member].name} just joined the leaderboard!`});
